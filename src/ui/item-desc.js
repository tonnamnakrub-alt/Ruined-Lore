...
function PASSIVES() {
  const RUSH = (ITEM_BY_ID.hwh && ITEM_BY_ID.hwh.ultRush) || { ad: 20, as: 0.3, ms: 0.15, dur: 8, cd: 30 };
  const ONHIT = (ITEM_BY_ID.kff && ITEM_BY_ID.kff.apOnHit) || { flat: 15, apRatio: 0.20 };
  return {
    ...
    hwh: tr("กดท่าไม้ตายแล้วได้ AD +{0} · ความเร็วโจมตี +{1}% · ความเร็วเดิน +{2}% นาน {3} วิ (ทุก {4} วิ)",
      RUSH.ad, Math.round(RUSH.as * 100), Math.round(RUSH.ms * 100), RUSH.dur, RUSH.cd),
    cbg: tr(
    ...
    kff: tr("ออโต้แถมดาเมจเวท {0} (+{1}% AP) ทุกครั้งที่ตีโดน", ONHIT.flat, Math.round(ONHIT.apRatio * 100)),
